...
def calculate_distances(boxes_np):
    ...
    for box in tqdm(boxes_np):
        ...
        distances_df = pd.concat([distances_df, pd.DataFrame({key:value for key, value in zip(columns, [box_names, box_1, distances])})])

    distances_df = distances_df[distances_df["distance"] > .0] # get rid of self distances
...
def day08_1(boxes_np, limit = 1000):
    ...
    circuits = [[distances_df.iloc[row_id]["box_1"], distances_df.iloc[row_id]["box_2"]]]
    ...
    # print(row_id, distances_df.iloc[row_id].values)
        ...
        if box_1_connected and box_2_connected and box_1_circuit == box_2_circuit:
            ...
            circuits.append([distances_df.iloc[row_id]["box_1"], distances_df.iloc[row_id]["box_2"]])
        connection_count += 1
        pbar.update(1)
...
def day08_2(boxes_np, limit = 1000):
    ...
    return int(distances_df.iloc[row_id-1]["box_1"].split(",")[0]) * int(distances_df.iloc[row_id-1]["box_2"].split(",")[0])
